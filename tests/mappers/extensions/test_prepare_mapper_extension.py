...
"""
    ...
    def test_insert_prepare_not_needed(self):
        cluster = "my-cluster"
        region = "europe-west3"
        job_properties = {"nameNode": "hdfs://localhost:8020"}
        config = {"dataproc_cluster": cluster, "gcp_region": region}
...
"""
        ...
        self.assertFalse(extension.has_prepare())
        original_tasks: List[Task] = [Task(task_id=TEST_MAPPER_NAME, template_name="pig.tpl")]
        original_relations: List[Relation] = []
        tasks, relations = extension.insert_prepare_if_needed(
            tasks=original_tasks, relations=original_relations
        )
        self.assertEqual(original_tasks, tasks)
        self.assertEqual(original_relations, relations)
        self.assertEqual("mapper", extension.first_task_id)

    def test_insert_prepare_needed(self):
        cluster = "my-cluster"
        region = "europe-west3"
        job_properties = {"nameNode": "hdfs://localhost:8020"}
        config = {"dataproc_cluster": cluster, "gcp_region": region}
...
"""
        pig_node = ET.fromstring(pig_node_prepare_str)
        extension = self.get_mapper_extension(
            node=pig_node, props=PropertySet(config=config, job_properties=job_properties)
        )
        self.assertTrue(extension.has_prepare())
        ...
        self.assertEqual([extension.get_prepare_task(), original_tasks[0]], tasks)
        self.assertEqual([Relation(from_task_id="mapper_prepare", to_task_id="mapper")], relations)
        self.assertEqual("mapper_prepare", extension.first_task_id)
