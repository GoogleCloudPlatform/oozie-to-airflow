...
class TestParsedNode(unittest.TestCase):
    def setUp(self):
        ...
        self.p_node.tasks = [Task(task_id="dummy", template_name="dummy.tpl")]
...
class TestParserNodeMultipleOperators(unittest.TestCase):
    def setUp(self):
        self.p_node = parsed_node.ParsedNode(None)
        self.p_node.tasks = [
            Task(task_id="first_task_id", template_name="dummy.tpl"),
            Task(task_id="second_task_id", template_name="dummy.tpl"),
        ]

    def test_first_task_id(self):
        self.assertEqual("first_task_id", self.p_node.first_task_id_in_correct_flow)

    def test_last_task_id(self):
        self.assertEqual("second_task_id", self.p_node.last_task_id)
