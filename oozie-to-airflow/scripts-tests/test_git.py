...
class GitTestCase(ShellScriptTestCase):
    def test_success_execution(self):
        with mock_app("gcloud"):
            return_code = self.run_bash_command(
                "./scripts/git.sh --git-uri GIT_URI --destination-path DEST_URI --region REGION "
                "--cluster CLUSTER"
            )
    ...
    def test_success_execution_with_custom_branch(self):
        with mock_app("gcloud"):
            return_code = self.run_bash_command(
                "bash ./scripts/git.sh --git-uri GIT_URI --destination-path DEST_URI --region REGION "
    ...
    @parameterized.expand(
        [
            "bash ./scripts/git.sh --destination-path DEST_URI --region REGION --cluster CLUSTER",
            "bash ./scripts/git.sh --git-uri GIT_URI --region REGION --cluster CLUSTER",
            "bash ./scripts/git.sh --git-uri GIT_URI --destination-path DEST_URI --cluster CLUSTER",
            "bash ./scripts/git.sh --git-uri GIT_URI --destination-path DEST_URI --region REGION ",
        ]
    )
