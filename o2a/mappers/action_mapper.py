...
class ActionMapper(BaseMapper, ABC):
    ...
    def on_parse_node(self, workflow: Workflow):
        ...
        self._parse_config(workflow)

    def _parse_config(self, workflow: Workflow):
        job_xml_nodes = find_nodes_by_tag(self.oozie_node, TAG_JOB_XML)
        new_properties = extract_properties_from_job_xml_nodes(
            job_xml_nodes=job_xml_nodes, input_directory_path=workflow.input_directory_path, props=self.props
        )
